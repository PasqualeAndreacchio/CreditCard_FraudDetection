...
Xtrain, Xtest, Ytrain, Ytest = preprocess.get_dataset(test_size=config.get("test_size"))
...
reconstructor = Complete_Autoencoder(config=config)

# Training configuration and definition
trainer = Trainer(model=reconstructor, config=config)
